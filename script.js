...
const showHistory = () => {
	...
	timesArr.forEach((time) => {
		...
		newTime.innerHTML = `Measurment nr ${num}: <span>${time}</span>`;
...
colorOne.addEventListener("click", () => {
	...
	root.style.setProperty("--hover-color", "rgb(173, 21, 13)");
});

colorTwo.addEventListener("click", () => {
	root.style.setProperty("--first-color", "rgb(6, 173, 250)");
	root.style.setProperty("--hover-color", "rgb(11, 109, 155)");
});

colorThree.addEventListener("click", () => {
	root.style.setProperty("--first-color", "rgb(0, 255, 42)");
	root.style.setProperty("--hover-color", "rgb(10, 153, 34)");
});

colorFour.addEventListener("click", () => {
	root.style.setProperty("--first-color", "rgb(174, 0, 255)");
	root.style.setProperty("--hover-color", "rgb(95, 13, 133)");
});
